...
本文档用于项目交接、二次开发、部署维护和线上排障。当前项目目录为 `question_bank_site/`，线上站点为 `https://caac.gengau.com/`。
...
当前已完成 P0/P1 级别升级：基础前端工程化、进度 schema v2、学习体验增强、Dashboard、移动端适配。模拟考试独立页面仍属于后续 P2。
...
- 模拟考试独立页面尚未实现，当前只有 AI 考试分析接口预留。
- 后台仍是旧版 `admin.html / admin.js`，尚未迁移到 React。
- 后台 token 当前仍等同于后台密码，安全性一般。
...
- 持久化考试记录到 `progress.examRecords`

P3：后台与题库维护

- 后台迁移到 React
- 题库上传前增加 schema 校验、重复 ID 检查和章节统计预览
- 用户管理增加搜索、分页、进度详情和考试记录查看
- AI 配置增加连接测试、错误摘要和模型参数说明
...
- 独立 admin session
- 错误日志和基础监控
- 将 progress JSON 拆分为 `question_progress`、`exam_records`、`daily_stats`
