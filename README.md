...
- `GET /admin`
- React 路由 fallback：`/`、`/practice`、`/dashboard`、`/exam`

## 已知限制

- 模拟考试 P2 已完成：已有 `/exam` 独立页、考试设置、倒计时、答题卡、交卷报告、AI 诊断和考试记录持久化。
- 后台仍基于旧版原生 `admin.html / admin.js`，已做视觉和功能升级，但尚未迁移到 React。
- 题库上传已可用，但上传前 schema 校验、重复 ID 检查、章节统计预览和 diff 仍需增强。
- 用户管理已有基础列表和操作，完整单用户进度详情、分页搜索和考试记录详情仍需补齐。
...
- `/exam` 独立页面。
- 自定义题量、时间、章节范围、随机抽题和目标正确率。
- 考试中隐藏答案和 AI 讲解，显示倒计时、进度、上一题/下一题和答题卡跳转。
- 交卷后展示分数、正确率、错题、薄弱章节和复习建议。
- 考试记录持久化到 `progress.examRecords`，Dashboard 显示最近考试结果和趋势。
- 考试报告 AI 诊断由用户点击按钮后生成。

P3：后台升级（部分完成）

- 已完成：后台视觉升级、AI 配置、AI 测试、题库上传、用户列表和基础统计。
- 待完成：后台迁移 React。
- 待完成：题库上传前严格校验、重复 ID 检查、章节统计预览和 diff。
- 待完成：用户搜索、分页、进度详情和考试记录详情。

P4：部署、安全和性能（部分完成）

- 已完成：Docker 多阶段构建、Vite 静态资源 hash、敏感/运行文件 `.gitignore` 排除。
- 部分完成：FastAPI 静态文件 no-cache 和基础接口日志。
- 待完成：系统化静态资源缓存策略。
- 待完成：API 字段白名单和 progress schema 服务端校验。
- 待完成：后台独立 session。
